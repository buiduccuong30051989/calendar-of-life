...
export const WeeksCalendar = ({ dates, dateDifferences, settings }) => {
  const weeks = Array.from({ length: dateDifferences.weeks }).map((_, index) => {
    ...
    const monthClasses = startDate.getMonth() === endDate.getMonth() ? startMonthClass : `${startMonthClass} ${endMonthClass}`;
    ...
    const yearClasses = startDate.getFullYear() === endDate.getFullYear() ? startYearClass : `${startYearClass} ${endYearClass}`;
  ...
  const rows = [];
  for (let i = 0; i < weeks.length; i += 56) {
    rows.push(weeks.slice(i, i + 56));
  }
    ...
    <table className="table-auto mt-4 mx-auto w-fit">
      <tbody>
        {rows.map((row, rowIndex) => (
          <tr key={rowIndex}>
            {row.map((week) => (
              <td key={week.index} className="p-1">
                <WeekItem {...week} />
              </td>
            ))}
          </tr>
